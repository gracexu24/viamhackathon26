...
def red_center(bgr):
    """Require one circular red candidate; return its bounding-box midpoint."""
    ...
    for contour in contours:
        ...
        if area < 80 or perimeter <= 0 or 4 * np.pi * area / perimeter**2 < .65:
            continue
        x, y, w, h = cv2.boundingRect(contour)
        if x <= 0 or y <= 0 or x+w >= bgr.shape[1] or y+h >= bgr.shape[0]:
            continue
        candidates.append(np.array([x+(w-1)/2, y+(h-1)/2]))
